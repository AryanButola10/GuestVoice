...
const INPUT_CLS =
  'w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 text-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500';
...
export default function Dashboard() {
  ...
  // ── Create form ────────────────────────────────────────────────────────────
  const [form, setForm]       = useState({ guest_name: '', property: '', rating: 5, review_text: '' });
  ...
  // ── Delete confirmation ────────────────────────────────────────────────────
  const [deleteTarget, setDeleteTarget]   = useState(null); // review id awaiting confirmation
  ...
  const fetchStats = useCallback(async () => {
    ...
    } catch { /* silent — stats failing should never break the page */ }
  }, []);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      if (res.status === 401) { toast.error('Session expired.'); logout(); navigate('/login'); return; }
  ...
  }, [form, token, logout, navigate, fetchReviews, fetchStats]);

  // ── Edit review ────────────────────────────────────────────────────────────
  const handleEditStart = useCallback((review) => {
    setEditingId(review.id);
    setEditForm({ guest_name: review.guest_name, property: review.property, rating: review.rating, review_text: review.review_text });
    setAiResults(prev => { const n = { ...prev }; delete n[review.id]; return n; });
  }, []);
  ...
  const handleEditSave = useCallback(async (id) => {
    if (!editForm.review_text?.trim() || editForm.review_text.length < 10) {
      toast.error('Review text must be at least 10 characters.');
      return;
    }
    if (!editForm.guest_name?.trim()) { toast.error('Guest name is required.'); return; }
    if (!editForm.property?.trim()) { toast.error('Property name is required.'); return; }
  ...
  }, [editForm, token, logout, navigate, fetchReviews, fetchStats]);

  // ── Delete review (with confirmation) ─────────────────────────────────────
  const handleDeleteConfirm = useCallback(async () => {
    setDeleteLoading(true);
    try {
      const res = await fetch(`${API_BASE}/reviews/${deleteTarget}`, {
        method: 'DELETE',
  ...
  }, [deleteTarget, token, logout, navigate, fetchStats]);

  // ── AI analyse ─────────────────────────────────────────────────────────────
  const handleAIAnalyse = useCallback(async (review) => {
    setAiLoading(prev => new Set(prev).add(review.id));
    try {
      const res = await fetch(`${API_BASE}/ai/analyze`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ review_text: review.review_text, guest_name: review.guest_name, property: review.property, rating: review.rating }),
      });
      if (res.status === 401) { toast.error('Session expired.'); logout(); navigate('/login'); return; }
      if (res.status === 429) { toast.error('AI rate limit reached. Please wait a moment.'); return; }
      if (!res.ok) { const e = await res.json(); throw new Error(e.detail || 'AI analysis failed.'); }
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            {[
              ...
              <div key={stat.label} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 shadow-sm">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-6 shadow-sm h-fit">
              <h2 className="text-slate-800 dark:text-slate-100 font-semibold text-sm mb-4">Submit a Review</h2>

              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Guest Name</label>
                  <input id="guest-name-input" type="text" required placeholder="e.g. Sarah Mitchell"
                ...
                <div>
                  ...
                  <input id="property-input" type="text" required placeholder="e.g. Mountain View Cottage"
                ...
                <div>
                  ...
                  <input id="rating-input" type="range" min="1" max="5" value={form.rating}
                    onChange={e => setForm({ ...form, rating: e.target.value })}
                    className="w-full accent-green-600" />
                </div>
                <div>
                  <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Review Text</label>
                  <textarea id="review-text-input" required rows={4} placeholder="Paste the guest review here..."
                    value={form.review_text} onChange={e => setForm({ ...form, review_text: e.target.value })}
                    className={`${INPUT_CLS} resize-none`} />
                </div>
                <div className="flex gap-3">
                  <button id="dashboard-analyse-btn" type="submit" disabled={submitting}
                    className="btn-primary disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
                    {submitting ? <><Loader size="sm" label="Submitting" /> Submitting…</> : 'Analyse Review'}
                  </button>
                  <button id="dashboard-clear-btn" type="button"
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {loadingReviews ? (
                ...
                /* ── Empty state ── */
                <div className="bg-white dark:bg-slate-800 border border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-12 text-center">
                  <div className="text-4xl mb-3">📋</div>
                  <p className="text-slate-700 dark:text-slate-200 font-semibold text-sm mb-1">No reviews yet</p>
                  <p className="text-slate-400 dark:text-slate-500 text-xs">
                    Submit your first guest review using the form on the left.
                  </p>
                </div>
                  ...
                  <div key={review.id}
                    ...
                    {editingId === review.id ? (
                      ...
                      <div className="space-y-3">
                        <p className="text-xs font-semibold text-green-600 dark:text-green-400 mb-2">✏️ Editing Review</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Guest Name</label>
                            <input type="text" value={editForm.guest_name}
                              onChange={e => setEditForm({ ...editForm, guest_name: e.target.value })}
                              className={INPUT_CLS} />
                          </div>
                          <div>
                            <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Property</label>
                            <input type="text" value={editForm.property}
                        ...
                        <div>
                          <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">
                            Rating: <span className="text-green-600 font-semibold">{editForm.rating} ★</span>
                          </label>
                          <input type="range" min="1" max="5" value={editForm.rating}
                            onChange={e => setEditForm({ ...editForm, rating: e.target.value })}
                            className="w-full accent-green-600" />
                        </div>
                        <div>
                          <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Review Text</label>
                          <textarea rows={3} value={editForm.review_text}
                            onChange={e => setEditForm({ ...editForm, review_text: e.target.value })}
                            className={`${INPUT_CLS} resize-none`} />
                        </div>
                        <div className="flex gap-2 pt-1">
                          <button onClick={() => handleEditSave(review.id)} disabled={editSubmitting}
                            className="btn-primary text-xs px-3 py-1.5 flex items-center gap-1.5 disabled:opacity-50">
                            {editSubmitting ? <><Loader size="sm" /> Saving…</> : 'Save Changes'}
                          </button>
                          <button onClick={handleEditCancel} className="btn-outline text-xs px-3 py-1.5">
                            Cancel
                          </button>
                        </div>
                      </div>
                      ...
                      <>
                        <div className="flex items-start justify-between gap-4 mb-2">
                          <div>
                            <p className="text-slate-900 dark:text-slate-100 font-semibold text-sm">{review.guest_name}</p>
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            >
                              {aiLoading.has(review.id) ? <><Loader size="sm" /> Analysing…</> : '🤖 AI Analyse'}
                            </button>
                            {/* Edit */}
                            <button
                              id={`edit-btn-${review.id}`}
                              onClick={() => handleEditStart(review)}
                              className="text-xs text-slate-400 hover:text-green-600 dark:hover:text-green-400 transition-colors font-medium"
                            >
                              ✏️ Edit
                            </button>
                            {/* Delete */}
                        ...
                        {aiResults[review.id] && (
                          <div className="mt-4 pt-4 border-t border-violet-100 dark:border-violet-900/30 space-y-3">
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                              ...
                              <div className="flex-1 bg-slate-100 dark:bg-slate-700 rounded-full h-1.5 min-w-[60px] max-w-24">
                            ...
                            <div className="bg-violet-50 dark:bg-violet-900/20 border border-violet-100 dark:border-violet-800 rounded-lg p-3">
                              ...
                              <button
                                onClick={() => { navigator.clipboard.writeText(aiResults[review.id].management_response); toast.success('Response copied!'); }}
      ...
      >
        ...
        <div className="flex gap-3 justify-end">
          ...
          <button
            ...
            disabled={deleteLoading}
            className="px-4 py-2 text-sm font-semibold bg-red-600 hover:bg-red-700 text-white rounded-lg disabled:opacity-50 flex items-center gap-2 transition-colors"
          >
            {deleteLoading ? <><Loader size="sm" /> Deleting…</> : 'Delete Review'}
          </button>
        </div>
      </Modal>
    </div>
